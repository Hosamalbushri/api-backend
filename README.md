...
| --- | --- | --- | --- | --- |
| `app/sync/service.py` `SyncService` | `app/Sync/SyncService.php` | Push/pull/meta, versions, soft-delete, upsert-as-create | COMPLETE | tests |
| `app/sync/router.py` | `app/Http/Controllers/Api/V1/SyncController.php` | HTTP contracts + savepoints for batch | COMPLETE | tests |
| `app/sync/schemas.py` | request validation + `SupportedEntities` | Entity types and payload shapes | COMPLETE | tests |
| `app/models/sync.py` | Eloquent `Sync*` / `Company` | JSON payloads, sequences, operation ledger | COMPLETE | tests |
| `app/auth/service.py` | `app/Auth/AuthService.php` | Login/refresh/logout/switch-company/devices | COMPLETE | tests |
...
| `app/admin/router.py` | `User/Role/Company/DeviceController` | Admin HTTP API | COMPLETE | tests |
| `app/audit/service.py` | `AuditService` | Audit rows, no secrets | COMPLETE | tests |
| `app/core/exceptions.py` | `app/Exceptions/*` | Same `error.code` JSON envelope | COMPLETE | tests |
| `app/core/rate_limit.py` | `SlidingWindowLimiter` + `AuthRateLimit` | Login/refresh 429 | COMPLETE | tests |
| `app/core/config.py` `assert_safe_for_environment` | `ProductionSettings` | Production refuse-to-boot | COMPLETE | tests |
